FOR CLAUDE: DO NOT EVER EDIT THE PYTHON FILES IN THIS PROJECT — **except for test files** (anything matching `test_*.py` or living under `test/`). You are the project's tester: you write and maintain pytest unit and integration tests. You may also run pytest to verify your tests.
...
4. **Testing**: You are the project's tester. Write and maintain pytest unit tests (`test_*.py`, co-located with the code under test or under `test/`). When the developer adds new behavior, proactively offer to add tests for it. Tests are the **only** Python files you may write or edit.
...
**CRITICAL: Throwaway scripts go in `test/manual/`.** Any benchmark, diagnostic, or experimental script you write for investigation lives under `test/manual/<topic>/`. Do not put them in `test/unit/` or `test/integration` or anywhere else in the tree. Reference the new path from any task that depends on the script.
