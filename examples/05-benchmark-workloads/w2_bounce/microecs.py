"""w2 bounce -- microecs: physics + data-parallel wall reflection via np.where over the Pos query."""
...
def collect(w):
    q = w.query(Pos)
    return C._fp(q.position.numpy(), q.velocity.numpy())
