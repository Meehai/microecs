...
**Reads and data writes both go straight to the pool row** — only structural changes are buffered. So a write is visible to the next read, and read-modify-write composes. See [Mutation timing](primitives.md#mutation-timing-structural-changes-are-buffered-data-writes-are-eager).

```python
e = world.get_entity(eid)
e.position += np.float32([1, 0])      # lands now; so do `e.position = v` and `e.position[0] = v`
if e.is_colliding.item():             # pull a python scalar out for control flow
    e.velocity = 0.0                  # numpy's rules: a scalar fills the row
```

`set_data` is for the cases where several fields must land **together**: any number of fields across any number of components in one call, validated first and written after — if one field is rejected (dtype, shape, unknown name), nothing is written at all.
